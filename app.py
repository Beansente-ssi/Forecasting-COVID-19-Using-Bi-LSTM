...
def index():
    ...
    if request.method == "GET":    
        ...
        actual_mae = mean_absolute_error(cases, preds)
        actual_rmse = sqrt(mean_squared_error(cases, preds))
        actual_r2 = r2_score(cases, preds)

        sma_mae = mean_absolute_error(sma, preds)
        sma_rmse = sqrt(mean_squared_error(sma, preds))
        sma_r2 = r2_score(sma, preds)
...
def get_result():
    if request.method == "POST":
        try:
            ...
            bool = forecast.validate_file(file)
